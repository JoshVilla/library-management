...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const countBorrowing = await BorrowBooks.countDocuments({ studentId: studentId, isApproved: STATUS.INPROGRESS });
    const countFailed = await BorrowBooks.countDocuments({ studentId: studentId, isApproved: STATUS.FAILED }); 
